...
广义线性模型：主要有线性回归和逻辑回归，但逻辑回归进行了重要的改造以适应分类任务，而不是回归任务。
...
逻辑回归是用于二分类任务的。它的目标是预测类别 y∈{0,1}。

不同于线性回归，线性回归直接预测连续纸，逻辑回归用于分类任务，因此，逻辑回归使用一个称为逻辑函数（sigmoid函数）的函数将线性组合的结果转换为一个概率值，该概率值表示输入属于某个类别的可能性。



**相似之处**

- **线性关系**: 两者都假设自变量与因变量之间存在线性关系（在逻辑回归中，线性关系通过sigmoid函数进行非线性变换）。
- **参数估计**: 两者都通过估计参数 β\betaβ 来找到数据之间的最佳拟合。
- **梯度下降**: 都可以通过梯度下降法来优化模型参数。
...
- 线性回归的输出是实数。
- 逻辑回归的输出是概率，通常映射到 {0,1}\{0, 1\}{0,1} 类别。
